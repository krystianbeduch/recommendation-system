...
const App: React.FC = () => {
    ...
    const loadUserById = async (userId: number): Promise<void> => {
        try {
            const response: AxiosResponse<RawUser> = await axios.get<RawUser>(`http://localhost:8000/api/users/${userId}`)
    ...
    const fetchMetadata = async (): Promise<void>  => {
        try {
            const [genresRes, languageRes]: [AxiosResponse<Genre[]>, AxiosResponse<Language[]>] = await Promise.all([
                axios.get<Genre[]>('http://localhost:8000/api/genres'),
                axios.get<Language[]>('http://localhost:8000/api/languages')
            ]);

            const genres = Object.fromEntries(genresRes.data.map(({ id, name }) => [id, name]));
            const languages = Object.fromEntries(languageRes.data.map(({ iso_639_1, name }) => [iso_639_1, name]));
    ...
    useEffect(() => {
        if (userData && Object.keys(genresMap).length > 0 && Object.keys(languagesMap).length > 0) {
